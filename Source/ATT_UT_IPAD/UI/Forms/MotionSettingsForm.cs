...
namespace ATT_UT_IPAD.UI.Forms
{
    public partial class MotionSettingsForm : Form
    {
        ...
        {
            Save();
            Logger.Write(LogType.GUI, "Clicked MotionSettingForm Save Button");
        }

        private void lblCancel_Click(object sender, EventArgs e)
        {
            this.Close();
            Logger.Write(LogType.GUI, "Clicked MotionSettingForm Close Button");
        }
        ...
        private void Save()
        {
            ...
            if (yesNoForm.ShowDialog() == DialogResult.Yes)
            {
                ...
                XCommonControl?.WriteChangeLog();
                Z1CommonControl?.WriteChangeLog();
                Z2CommonControl?.WriteChangeLog();

                XVariableControl?.WriteChangeLog();

                if (ParamTrackingLogger.IsEmpty == false)
                {
                    ParamTrackingLogger.AddLog("All Motion Parameter saved.");
